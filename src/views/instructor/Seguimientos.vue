...
// Seguimientos filtrados
const seguimientosFiltrados = computed(() => {
  return seguimientos.value.filter(seg => {
    const matchAprendiz = seg.aprendiz.toLowerCase().includes(filtroAprendiz.value.toLowerCase())
    const matchFicha = seg.ficha.includes(filtroFicha.value)
    const matchPrograma = seg.programa.toLowerCase().includes(filtroPrograma.value.toLowerCase())
    const matchEstado = seg.estado.toLowerCase().includes(filtroEstado.value.toLowerCase())
    
    return matchAprendiz && matchFicha && matchPrograma && matchEstado
  })
})
...
const guardarNuevo = () => {
  ...
  console.log('Guardando nuevo seguimiento:', nuevoSeguimiento.value)
  showModalNuevo.value = false
...
const visualizarPlantilla = (plantilla) => {
  console.log('Visualizar plantilla:', plantilla.nombre)
...
const descargarPlantillaVisualizada = () => {
  console.log('Descargar plantilla visualizada')
}

const eliminarSeguimiento = (row) => {
  console.log('Eliminar seguimiento:', row)
}

const visualizarSeguimiento = (row) => {
  console.log('Visualizar seguimiento:', row)
}

const descargarSeguimiento = (row) => {
  console.log('Descargar seguimiento:', row)
}
...
:deep(.success-notification) {
  ...
  border-left: 5px solid #39a900 !important;
  color: #333 !important;
