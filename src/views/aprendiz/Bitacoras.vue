<template>
  <div class="page-content">
        ...
        <div class="plantillas-grid">
          <div v-for="template in templates" :key="template.id" class="plantilla-card">
            ...
            <div class="plantilla-acciones">
              <q-btn dense flat round icon="visibility" color="grey-7" @click="visualizarPlantilla(template)">
                <q-tooltip>Visualizar</q-tooltip>
              </q-btn>
              <q-btn dense flat round icon="file_download" color="grey-7" @click="descargarPlantilla(template)">
    ...
    <q-page-sticky v-if="activeTab === 'subir'" position="bottom-left" :offset="[30, 30]">
      <q-btn fab icon="add" size="lg" @click="abrirDialogSubir" style="background-color: #44b900; color: white;">
        <q-tooltip>Subir bitácoras</q-tooltip>
      </q-btn>
    </q-page-sticky>

    <ModalBase ref="uploadModal">
      <template #header>
      ...
      <template #footer>
        <div class="modal-footer-buttons">
          <BotonCerrar @click="cerrarDialogSubir" />
          <BotonEnviar 
...
<script setup>
import { ref } from 'vue'
import Tabla from 'src/components/tables/maintable.vue'
...
// Métodos principales
const descargar = (row) => console.log('Descargar:', row)
const visualizar = (row) => console.log('Visualizar:', row)
const descargarPlantilla = (template) => console.log('Descargar plantilla:', template)
const visualizarPlantilla = (template) => console.log('Visualizar plantilla:', template)
...
const subirBitacoras = async () => {
  ...
  try {
    ...
    selectedFiles.value.forEach(file => formData.append('bitacoras', file))
    
    console.log('Archivos subidos:', selectedFiles.value)
    cerrarDialogSubir()
    
  } catch (error) {
    console.error('Error al subir bitácoras:', error)
  } finally {
