...
export function useColombia() {
  // Ref para el departamento seleccionado
  const departamentoSeleccionado = ref('')
  
  // Lista de departamentos
  const departamentos = computed(() => {
    return colombiaData.map(item => ({
  ...
  const ciudades = computed(() => {
    ...
    const dept = colombiaData.find(item => item.departamento === departamentoSeleccionado.value)
  ...
  const getCiudadesByDepartamento = (departamento) => {
    const dept = colombiaData.find(item => item.departamento === departamento)
    return dept ? dept.ciudades.sort() : []
  }
  
  // Función para validar si una ciudad pertenece a un departamento
  const validarCiudadDepartamento = (ciudad, departamento) => {
    const dept = colombiaData.find(item => item.departamento === departamento)
    return dept ? dept.ciudades.includes(ciudad) : false
  }
  
  // Función para buscar departamento por ciudad
  const getDepartamentoByCiudad = (ciudad) => {
    const dept = colombiaData.find(item => 
      item.ciudades.some(c => c.toLowerCase() === ciudad.toLowerCase())
    )
